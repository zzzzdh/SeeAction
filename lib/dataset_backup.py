...
def create_dataset():
  ...
  for row in val_rows:
    ...
    if stride != stride_2 and count < len(val_rows) * salt:
      label_list_train.append(csv_file + ' ' + stride_2 + ' ' + verb_class + ' ' + noun_class + ' ' + adv_class + '\n')
      count += 1
  print ('Val rows: {}, add salt: {}'.format(len(val_rows), count))
